/**
 * Master Customers Worklist
 * Displays master customer companies with task progress and billing contact info
 */
...
export default function MasterCustomersWorklist() {
  ...
  const columns: DataTableColumn<CompanyView>[] = [
    ...
    { id: 'accountManager', label: 'Account Manager', sortable: true, width: 150 },
  ...
  const filteredCustomers = useMemo(() => {
    ...
    return rows.filter((company) =>
      [company.name, company.clientId, company.accountManager, company.billingContactEmail]
        .filter(Boolean)
  ...
  return (
    <div className="mt-4 space-y-4">
      <PageToolbar>
        <Input
          placeholder="Search master customers..."
          value={search}
      ...
      <DataTable
        ...
        loading={isLoading}
        error={error ? 'Failed to load master customers' : null}
        ...
        onRefresh={refetch}
        title={`${search ? filteredCustomers.length : data?.total || 0} Master Customers`}
