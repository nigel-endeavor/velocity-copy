...
import com.vertek.corporate.qto.solution.SolutionManager;
...
@MessageDriven(activationConfig = {
        ...
        @ActivationConfigProperty(propertyName = "destination",
                propertyValue = QuoteProcessingListener.QUOTE_PROCESSING_QUEUE),
        @ActivationConfigProperty(propertyName = "acknowledgeMode", propertyValue = "Auto-acknowledge"),
        @ActivationConfigProperty(propertyName = "maxSession", propertyValue = "1")
})
public class QuoteProcessingListener implements MessageListener {

    /** Private Logger for this class. */
    private static final Logger LOGGER = LoggerFactory.getLogger(QuoteProcessingListener.class);

    /** QuoteProcessingQueue. */
    public static final String QUOTE_PROCESSING_QUEUE = "java:/queue/qto.QuoteProcessingQueue";
    ...
    public void onMessage(final Message message) {
        ...
        try {
            ...
            //retrieve persisted quote
            Quote quote = quoteManager.retrieve(quoteId);

            //retrieve order if it already exists
            Order order = orderManager.getByQuoteId(quote.getVendorQuoteId());

            //if the order doesn't exist, we're creating the order, then locations/services
            //else, then create the locations/services
