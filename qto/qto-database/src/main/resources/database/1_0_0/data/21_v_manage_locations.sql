DROP VIEW IF EXISTS v_manage_sites CASCADE;
CREATE OR REPLACE VIEW v_manage_sites AS
SELECT o.order_id,
       l.location_id,
       company_name,
       provisioner,
...
FROM company c
     ...
     LEFT JOIN (SELECT s.location_id,
                       ...
                       string_agg(CASE WHEN NOT bs.service_id IS NULL
			                            THEN 'Broadband'
		                            WHEN NOT ds.service_id IS NULL
			                            THEN 'DIA'
