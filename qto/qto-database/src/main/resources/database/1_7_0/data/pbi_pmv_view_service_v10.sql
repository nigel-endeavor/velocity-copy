...
SELECT l.client_location_id AS 'Client Service ID',
       ...
       DATE_FORMAT(
       case when hour(aa.field_tech_check_in) = 0 and minute(aa.field_tech_check_in) = 0 and second(aa.field_tech_check_in) = 0
				 then aa.field_tech_check_in
       when hour(aa.field_tech_check_in) = 4 and minute(aa.field_tech_check_in) = 0 and second(aa.field_tech_check_in) = 0
         then DATE_ADD(aa.field_tech_check_in, INTERVAL -4 HOUR)
       else DATE_ADD(aa.field_tech_check_in, INTERVAL -5 HOUR) end, '%m/%d/%Y %h:%i:%s %p') AS 'LEC Tech Check In',
       DATE_FORMAT(
       case when hour(aa.field_tech_check_out) = 0 and minute(aa.field_tech_check_out) = 0 and second(aa.field_tech_check_out) = 0
				 then aa.field_tech_check_out
       when hour(aa.field_tech_check_out) = 4 and minute(aa.field_tech_check_out) = 0 and second(aa.field_tech_check_out) = 0
         then DATE_ADD(aa.field_tech_check_out, INTERVAL -4 HOUR)
       else DATE_ADD(aa.field_tech_check_out, INTERVAL -5 HOUR) end, '%m/%d/%Y %h:%i:%s %p') AS 'LEC Tech Check Out',
