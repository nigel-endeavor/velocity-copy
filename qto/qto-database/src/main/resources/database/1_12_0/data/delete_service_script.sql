...
create temporary table surcharge_ids as (select surcharge_id from service_surcharge where service_id = @service_id);
...
create temporary table dispatch_ids as (select ftdi_dispatch_id from ftdi_dispatch d join activation_schedule s on d.schedule_id = s.activation_schedule_id where s.service_id = @service_id);
...
create temporary table activation_attempt_ids as (select activation_attempt_id from activation_attempt where service_id = @service_id);
...
create temporary table activation_schedule_ids as (select activation_schedule_id from activation_schedule where service_id = @service_id);
...
create temporary table custom_field_value_ids as (select custom_field_value_id from service_custom_field_value where service_id = @service_id);
...
create temporary table note_ids as select note_id from service_note where service_id = @service_id;
create temporary table dispute_ids as (select dispute_id from dispute where service_id = @service_id);
insert into note_ids (note_id) select note_id from dispute_note where dispute_id in (select dispute_id from dispute_ids);
create temporary table jeop_ids as (select  ji.jeop_instance_id from jeop_instance ji join service_jeop_instance sji on ji.jeop_instance_id = sji.jeop_instance_id where service_id = @service_id);
...
create temporary table file_attachment_ids as select file_attachment_id from service_file_attachment where service_id = @service_id;
...
create temporary table interval_ids as (select interval_instance_id from service_interval_instance where service_id = @service_id);
...
create temporary table jeop_instance_ids as (select jeop_instance_id from service_jeop_instance where service_id = @service_id);
...
create temporary table equipment_ids as (select equipment_id from service_equipment where service_id = @service_id);
...
create temporary table milestone_instance_ids as (select milestone_instance_id from service_milestone_instance where service_id = @service_id);
...
create temporary table shipment_tracking_ids as	(select shipment_tracking_id from service_shipment_tracking st where service_id = @service_id);
...
delete from dispute where service_id = @service_id;
delete from cost_history where service_id = @service_id;
...
# delete from pending_disconnect where parent_service_id = @service_id;
# alter table pending_disconnect
...
delete from service_brokerage where service_id = @service_id;
delete from service_snapshot where service_id = @service_id;

delete from 4g5g_service where service_id = @service_id;
delete from broadband_service where service_id = @service_id;
delete from cross_connect_service where service_id = @service_id;
delete from dia_service where service_id = @service_id;
delete from ethernet_service where service_id = @service_id;
delete from television_service where service_id = @service_id;
delete from ucaas_service where service_id = @service_id;
delete from mpls_service where service_id = @service_id;
alter table service
