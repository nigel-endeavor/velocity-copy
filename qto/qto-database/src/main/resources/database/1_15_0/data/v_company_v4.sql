...
SELECT lct.complete_date,
       ...
       act.value                                                                        as active_task,
       act.assigned_to                                                                  as active_task_assigned_to,
...
#          get the last completed task
         LEFT JOIN company_task lct
                   on lct.company_id = c.company_id and lct.company_task_id = (select ct2.company_task_id
                                                                               ...
                                                                               limit 1)
#          LEFT JOIN (select * from company_task ct where ct.company_id = c.company_id order by complete_date desc limit 1) lct on cts.last_completed_task_date = lct.complete_date
         LEFT JOIN (select *
                    from company_task ct
                    where ct.complete_date is null
                    order by ct.company_task_id asc
                    limit 1) act on act.company_id = c.company_id
