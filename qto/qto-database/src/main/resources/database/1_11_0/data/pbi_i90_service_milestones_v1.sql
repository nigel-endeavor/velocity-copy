DROP VIEW IF EXISTS pbi_i90_service_milestone CASCADE;
CREATE OR REPLACE VIEW pbi_i90_service_milestone AS
SELECT smi.service_id,
       ...
       CAST(MAX(CASE WHEN (m.milestone_code = 'CROSS_CONNECT_COMPLETE')
	                     THEN CONVERT_TZ(mi.milestone_date, 'UTC', 'US/Eastern')
                     ELSE NULL END) AS date) AS `Cross Connect Complete`,
       ...
       CAST(MAX(CASE WHEN (m.milestone_code = 'CUSTOMER_REQUESTED_DISCONNECT')
	                     THEN CONVERT_TZ(mi.milestone_date, 'UTC', 'US/Eastern')
                     ELSE NULL END) AS date) AS `Customer Requested Disconnect`,
       ...
       CAST(MAX(CASE WHEN (m.milestone_code = 'FIRST_VENDOR_INVOICE')
	                     THEN CONVERT_TZ(mi.milestone_date, 'UTC', 'US/Eastern')
                     ELSE NULL END) AS date) AS `1st Vendor Invoice date`,
       ...
       CAST(MAX(CASE WHEN (m.milestone_code = 'PROVIDER_DISCONNECT_COMPLETE')
	                     THEN CONVERT_TZ(mi.milestone_date, 'UTC', 'US/Eastern')
                     ELSE NULL END) AS date) AS `Provider Disconnect Complete`,

       CAST(MAX(CASE WHEN (m.milestone_code = 'PROVIDER_ORDER_SUBMITTED')
	                     THEN CONVERT_TZ(mi.milestone_date, 'UTC', 'US/Eastern')
                     ELSE NULL END) AS date) AS `Provider Order Submitted`,

       CAST(MAX(CASE WHEN (m.milestone_code = 'QA_CHECK_OPEN')
	                     THEN CONVERT_TZ(mi.milestone_date, 'UTC', 'US/Eastern')
                     ELSE NULL END) AS date) AS `QA Check Open`,

       CAST(MAX(CASE WHEN (m.milestone_code = 'RECEIVED')
	                     THEN CONVERT_TZ(mi.milestone_date, 'UTC', 'US/Eastern')
                     ELSE NULL END) AS date) AS `Service Received`,

       CAST(MAX(CASE WHEN (m.milestone_code = 'RETURNED_TO_ORDER_GROUP')
	                     THEN CONVERT_TZ(mi.milestone_date, 'UTC', 'US/Eastern')
                     ELSE NULL END) AS date) AS `Returned to Order Group`,

       CAST(MAX(CASE WHEN (m.milestone_code = 'RETURNED_TO_SALES')
	                     THEN CONVERT_TZ(mi.milestone_date, 'UTC', 'US/Eastern')
                     ELSE NULL END) AS date) AS `Returned to Sales`,
...
FROM milestone_instance mi
     ...
     JOIN service s ON smi.service_id = s.service_id
