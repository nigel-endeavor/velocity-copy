...
SELECT l.client_location_id AS 'Client Service ID',
       ...
       IF(s.legacy_dia_id IS NULL, s.service_id, s.legacy_dia_id) AS 'Site ID',
       (SELECT DATE_FORMAT(MIN(CONVERT_TZ(mi.milestone_date, 'UTC', 'US/Eastern')), '%m/%d/%Y') AS received_date
        FROM service svc
        ...
        WHERE m.milestone_code = 'RECEIVED'
		      AND svc.location_id = l.location_id
...
FROM location l
     ...
     JOIN (SELECT location_id, service_id, service_type, service_status, media_type, carrier, carrier_circuit_id, speed,
                  carrier_order_num, lan_ips, lan_subnet, lan_gateway, dns1, dns2, has_icb, legacy_dia_id
           FROM service s
           WHERE (service_id IN
                  (SELECT service_id
                   FROM service
     ...
     JOIN broadband_service bs ON s.service_id = bs.service_id
