...
SELECT
    ...
    IFNULL(svc.count_services_complete, 0) AS count_services_complete,
    IFNULL(svc.count_services_cancelled, 0) AS count_services_cancelled,
    IFNULL(svc.count_services_change_in_assignment, 0) AS count_services_change_in_assignment,
    IFNULL(svc.active_complete_mrc, 0) AS active_complete_mrc,
    IFNULL(svc.active_complete_nrc, 0) AS active_complete_nrc,
...
FROM company c
    ...
    SELECT
        ...
        COUNT(service_mrc) AS active_complete_mrc,
        COUNT(service_nrc) AS active_complete_nrc
    FROM service
    WHERE service_status = 'Service Complete' AND active = true
    GROUP BY location_id
) svc ON l.location_id = svc.location_id
