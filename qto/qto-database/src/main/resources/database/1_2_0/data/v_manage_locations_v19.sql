...
SELECT
    ...
    (SELECT MIN(inventory_added_date) from v_manage_services where location_id = l.location_id) as inventory_added_date
FROM company c
    ...
    FROM v_manage_services vms
    WHERE vms.inventory_added_date AND active = true
    GROUP BY location_id
