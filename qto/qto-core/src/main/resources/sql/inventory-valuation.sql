...
JOIN (
    SELECT
        ...
        DATE_FORMAT(snapshot_date, '%Y-%m') AS month_year
    FROM service_snapshot
    GROUP BY month_year
) ss_max ON DATE_FORMAT(ss.snapshot_date, '%Y-%m') = ss_max.month_year
    AND ss.snapshot_date = ss_max.max_date
WHERE
