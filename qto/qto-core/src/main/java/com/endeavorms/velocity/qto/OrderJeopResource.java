...
public class OrderJeopResource extends AbstractResource<JeopUnionView> {
    ...
    public ResponseEntity<?> getOrderJeops(@ModelAttribute final JeopUnionViewSearchCriteria criteria) {
    ...
    public ResponseEntity<?> create(@RequestBody final OrderJeop jeop) {
        try {
            if (jeop.getOrderId() == null) {
                throw new IllegalArgumentException("Missing orderId.");
            }
            OrderJeop returnedJeop = manager.create(jeop);
    ...
    @PutMapping("/{id}")
    @PreAuthorize("hasAnyAuthority('inventory:write','order:write')")
    public ResponseEntity<?> edit(@PathVariable("id") final Long id, @RequestBody final OrderJeop jeop) {
        try {
            ...
            OrderJeop returnedJeop = manager.edit(jeop);
            return ResponseEntity.ok(returnedJeop);
        } catch (Exception e) {
            return ResponseEntity.internalServerError().body(e.getMessage());
        }
