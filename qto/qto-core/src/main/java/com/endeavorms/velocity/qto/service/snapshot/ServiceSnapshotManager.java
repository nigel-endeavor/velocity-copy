...
public class ServiceSnapshotManager extends StandardManager<ServiceSnapshot> {
    ...
    public void createServiceSnapshots() {
        ...
        try {
            List<TenantView> tenants = transactionTemplate.execute(status -> tenantViewManager.getAllTenants());
            LOGGER.debug("Found {} tenants: {}", tenants.size(), tenants.stream().map(TenantView::getName).toArray());
            Date today = new Date();

            for (TenantView tenant : tenants) {
                Boolean skipTenant = transactionTemplate.execute(status -> {
                    ...
                    return configProperty != null && Boolean.parseBoolean(configProperty.getValue());
                });
                if (Boolean.TRUE.equals(skipTenant)) {
                    LOGGER.debug("Service snapshots are disabled for tenant: {}", tenant.getName());
                    continue;
                }

                List<Service> services = transactionTemplate.execute(status -> serviceManager.findByTenantId(tenant.getId()));
